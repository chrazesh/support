...
<head>
    ...
    <title>Pending Client-Issue</title>
</head>
<body>
  ...
  <div class="modal fade" id="exampleModal"  role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content" style="height:95vh;overflow:scroll;">
            ...
            <?php  
            $con = dbConnection();
            $aaaa="select * from support where user='".$_SESSION['username']."'";//echo $aaaa;
            $passaaaa=mysqli_query($con,$aaaa);
            $r=mysqli_fetch_assoc($passaaaa);
            $select="select * from issue where client_status='Open' and assigned='1' and assigned_officer='".$_SESSION['username']."'order by entryDate desc";
            $query=mysqli_query($con, $select);
            $i=1;
            while( $result = mysqli_fetch_assoc($query)){
              ?>
              <tr class="clstrCustomer" data-cId="<?php echo $result['customerId']?>" data-company="<?php echo $result['companyName']?>" data-token="<?php echo $result['id']?>" data-date="<?php echo $result['date']?>" data-issue="<?php echo $result['issue']?>" data-assigned="<?php echo $result['assigned_officer']?>" data-client_status="<?php echo $result['client_status']?>">       
                    <td><?php echo $i?></td>
  ...
  <button type="button" class="btn btn-primary mx-2 btnAdd">ADD</button>
    ...
    <div class="fromTo text-center">
      ...
      <input type="date" class="m-1 px-1" id="to" name="to" >
       <input type="hidden" class="m-1 px-1" id="filPending" value="<?php echo $_SESSION['username'] ?>" >
       
      <input type="button"  value="Search" onclick="GetReportBy('pending')"><br><br>
    </div>
    ...
    var filPending=$("#filPending").val();
     if(from=="" || to==""){
         ...
         $.ajax({
              ...
              method:"POST",
              data:{reportBy:ReportBy,from:from,to:to,filPending:filPending},
...
if(isset($_POST['Save'])){
  ...
  $select="select * from support where token='$token'";
...
//   echo $row5['id'];
       echo"<script>alert('Issue added to support');
               window.location.href='pending.php';
               </script>";
  }else{
