...
<div class="mx-4 heading">
    ...
    $id=$_GET['id'];
     $con = dbConnection();
    $selectCompany="select * from support where id='$id'";
...
<!--</div>-->
   ...
   <table class="display">
    ...
    <tr>
          ...
          <th>Company Name</th>
            <th>Attachment</th>
             ...
             <th>Status</th>
              <th>Remark</th>
               <th>Feedback</th>
                 <th>Feedback By(Support Staff)</th>
                <th>Feedback Date</th>
               <th>Forward To</th></th>
      ...
      <?php 
       $con = dbConnection();
      $id=$_GET['id'];
      $selectToken="select * from support where id='$id'";
      $passToken=mysqli_query($con,$selectToken);
      $fetchToken=mysqli_fetch_assoc($passToken);
      $fetchedToken=$fetchToken['token'];
      $fetchedSupport=$fetchToken['support_staff'];
    // $fetchedSupport="";
       if($_SESSION['username']=='oms'){
              $select="select activity.*,issue.client_status,issue.assigned_officer from activity left join issue on issue.id=activity.issueId where issueId='$fetchedToken'"; 
       }else{
           
      $select="select issue.*,activity.asssignedTo,activity.remarks,activity.user as actUser  from activity left join issue on activity.issueId=issue.id where activity.issueId='$fetchedToken' and activity.user='".$_SESSION['username']."' order by entryDate desc"; 
      ...
      <tr>
        <td><?php echo $i ?></td>
        <td><?php echo $fetchToken['token'] ?></td>
  ...
  <?php
        ...
        <?php
        if($result['client_status']=='Open'){
        if($result['assigned_officer']=="Not assigned to anyone"){ ?>
            <td>Issue is on review.</td>
            <?php
        }else{ ?>
         <td>Issue is on review by <?php echo $result['assigned_officer'] ?></td>
         <?php
        }
    
        }else{ ?>
         <td>Issue has been resolved.</td>
         <?php
        }
...
if(isset($_POST['Save'])){
  ...
  $insertquery="INSERT INTO `issue`(`companyName`, `date`, `path`, `attachment`, `issue`, `user`, `entryDate`) values('$companyname','$sdate','$path','$pname','$sissue','$user',now())";
