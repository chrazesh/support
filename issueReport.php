...
<!--</div>-->
  <div class="mx-4" data-aos="zoom-in-up">
   ...
   <table class="display">
    ...
    <tr>
      ...
      $select="select issue.*,activity.id as aId,activity.asssignedTo,activity.remarks,activity.user as aUser,activity.date as aDate from issue left join activity on activity.issueId=issue.id where issue.id='$id'"; 
      $query=mysqli_query($con, $select);
    
      $i=1;
      while( $result = mysqli_fetch_assoc($query)){
        //   $selectRemark="select * from activity left join issue on activity.issueId=issue.id";
        //   $passRemark=mysqli_query($con,$selectRemark);
        //   $rrr=mysqli_fetch_assoc($passRemark);
        ?>
  ...
  <?php
        ...
        if($result['assigned_officer']=="Not assigned to anyone"){ ?>
            <td>Your issue is on review.</td>
            <?php
        }else{ ?>
         <td>Your issue is on review by <?php echo $result['assigned_officer'] ?></td>
         <?php
        }
